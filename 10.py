...
def overlapping(a_list,b_list):
	...
	while i<len(x1):
		for num in x1:
			# if num in x2:			# Using the 'in' operator to verify if the member belongs to the other list
			if (num==x2[i]):
				print ("\nThere's at least 1 member repeated -->",num)	# when find the first repeated member, prints it 
